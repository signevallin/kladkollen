...
// kind väljer kvot-hink: 'outfit' för AI-outfits, 'trip' för resepackningar.
// Hinkarna är skilda åt – en packning ska inte äta av outfitkvoten.
export async function useAiCredit(request: Request, kind: 'outfit' | 'trip' = 'outfit'): Promise<boolean> {
  const token = (request.headers.get('authorization') || '').replace(/^Bearer\s+/i, '')
  const supabaseUrl = process.env.SUPABASE_URL || process.env.EXPO_PUBLIC_SUPABASE_URL
  const anonKey = process.env.SUPABASE_ANON_KEY || process.env.EXPO_PUBLIC_SUPABASE_ANON_KEY
  ...
  try {
    const res = await fetch(`${supabaseUrl}/rest/v1/rpc/use_ai_credit`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', apikey: anonKey, Authorization: `Bearer ${token}` },
      body: JSON.stringify({ max_free: kind === 'trip' ? FREE_TRIPS_PER_WEEK : FREE_AI_PER_WEEK, window_seconds: WEEK_SECONDS, quota_kind: kind }),
    })
